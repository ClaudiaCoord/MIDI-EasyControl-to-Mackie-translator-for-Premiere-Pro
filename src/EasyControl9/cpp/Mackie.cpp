...
bool Mackie::SelectorTarget(MidiUnit* unit, MIDIDATA& m) {
...
std::string Mackie::MIDIDATA::Print() {
	return LogString() << "\ttype:" << static_cast<uint32_t>(data[0])
		<< "id:" << static_cast<uint32_t>(data[1]) << ", value:" << static_cast<uint32_t>(data[2]);
}

...
namespace Common {
    namespace MIXER {
        ...
        AudioSessionEvents::AudioSessionEvents(AudioSessionList* ptr, std::function<void()> fdel, std::function<const GUID()> fguid)
            ...
            if (ptr == nullptr)
                throw runtime_werror(AudioSessionErrors::GetMsg(AudioSessionErrors::ERRMSG_LISTISNULL));
        }
        ...
        STDMETHODIMP AudioSessionEvents::OnSessionDisconnected(AudioSessionDisconnectReason t) {
            to_log::Get() << (log_string() << AudioSessionErrors::GetMsg(AudioSessionErrors::ERRMSG_SESSIONDISCONNECTED) << t);
            return S_OK;
        }
    }
}
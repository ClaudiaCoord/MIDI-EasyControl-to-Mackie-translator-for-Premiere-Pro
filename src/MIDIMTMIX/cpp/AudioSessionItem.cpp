			...
			try {
				...
				if (ptrse__) {
					...
					if (b && !isregistredse__) {
						...
						Common::to_log::Get() << (log_string() << L"Audio session item \"" << Name << "\" - bad \"SC\" registred state: " << isregistredse__.load() << L"/" << b);
					}
				}
				if (ptrae__) {
					...
					if (b && !isregistredae__) {
						...
						Common::to_log::Get() << (log_string() << L"Audio end point item \"" << Name << "\" - bad \"AE\" registred state: " << isregistredae__.load() << L"/" << b);
					}
				}

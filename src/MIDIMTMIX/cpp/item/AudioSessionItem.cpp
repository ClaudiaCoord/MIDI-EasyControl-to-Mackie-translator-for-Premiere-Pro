	...
	namespace MIXER {
			...
			try {
				...
				if (ptrse__) {
					...
					if (b && !isregistred_se__) {
						...
						to_log::Get() << (log_string() << __FUNCTIONW__
							<< AudioSessionErrors::GetMsg(AudioSessionErrors::ERRMSG_ITEM_BEGIN)
							<< Item.App.get<std::wstring>()
							<< AudioSessionErrors::GetMsg(AudioSessionErrors::ERRMSG_ITEM_SC)
							<< isregistred_se__.load() << L"/" << b);
					}
				}
				if (ptrae__) {
					...
					if (b && !isregistred_ae__) {
						...
						to_log::Get() << (log_string() << __FUNCTIONW__
							<< AudioSessionErrors::GetMsg(AudioSessionErrors::ERRMSG_ITEM_BEGIN)
							<< Item.App.get<std::wstring>()
							<< AudioSessionErrors::GetMsg(AudioSessionErrors::ERRMSG_ITEM_AE)
							<< isregistred_ae__.load() << L"/" << b);
					}
				}
		...
		log_string AudioSessionItem::to_string() {
			try {
				log_string s{};
				if (Item.IsEmptyApp()) {
					s << AudioSessionErrors::GetMsg(AudioSessionErrors::EMPTY);
				}
				else {

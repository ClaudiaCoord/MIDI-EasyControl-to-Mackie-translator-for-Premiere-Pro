    ...
    namespace MIDIMT {
        ...
        const bool CheckRun::parse_() {
            try {
                ...
                for (int32_t i = 0; i < na; i++) {
                    ...
                    std::wstring confpath = xargs.data[i];
                    if (confpath.starts_with(L"--conf=") && confpath.ends_with(L".cnf")) {
                        confpath__ = confpath.substr(7);
                        break;
                    }
                }
        ...
        COPYDATASTRUCT* CheckRun::build(uint32_t id, std::wstring s) {
            try {
                ...
                data->dwData = id;
                data->cbData = (s.size() + 1) * sizeof(wchar_t);
                data->lpData = new wchar_t[data->cbData] {};
                if (data->lpData)

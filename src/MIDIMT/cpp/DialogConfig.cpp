...
namespace Common {
	namespace MIDIMT {
		...
		void DialogConfig::HelpCategoryAddTarget(HWND hwnd, uint32_t idx) {
			log_string ls;
			ls << idx << Gui::GetBlank(idx) << L" - (Premiere Pro) " << MIDI::MackieHelper::GetTarget(idx).data();
			HelperAdd(hwnd, idx, ls.str());
		}
		void DialogConfig::HelpCategoryAddMMKey(HWND hwnd, uint32_t idx) {
			log_string ls;
			ls << idx << Gui::GetBlank(idx) << L" - (Multimedia Key) " << MIDI::MackieHelper::GetTranslateMMKey(idx).data();
			HelperAdd(hwnd, idx, ls.str());
		}
			...
			try {
				...
				for (uint32_t i = 0, n = 0; i <= static_cast<uint32_t>(MIDI::Mackie::Target::VOLUMEMIX); i++) {
					if ((k == IDC_SETUP_RADIO2) || (k == IDC_SETUP_RADIO1)) {
						if (((i >= 23) && (i <= 65)) || (i == 2) || (i == 3)) {
							HelpCategoryAddTarget(hwcl, i);
							continue;
						}
					}
					...
					if ((k == IDC_SETUP_RADIO1) && ((i == 253) || (i == 254)))
						HelpCategoryAddTarget(hwcl, i);
				}
			}
			catch (...) {
				...
				for (size_t i = 1; i < lv__->ListViewColumns(); i++) {
					...
					switch (i) {
						...
						case 4: {
							...
							ws = LangInterface::Get().GetString(MIDI::MackieHelper::GetTargetID(last__->unit.target));
							break;
						}
						case 5: {
							wn = std::to_wstring(static_cast<uint32_t>(last__->unit.longtarget));
							if (last__->unit.target == MIDI::Mackie::Target::MEDIAKEY)
		...
		void DialogConfig::ButtonSave() {
			...
			try {
				...
				Gui::SetControlEnable(hwnd, IDC_SETUP_SAVE, false);
